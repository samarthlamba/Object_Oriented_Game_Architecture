...
public abstract class Game implements GamePlay {
  ...
  /**
   * Game constructor initializes variables through parameter input and constant values retrieved
   * from the beans
   *
   ...
   */
  public Game(Player player, Collection<Unmovable> obstacles, Collection<Movable> entities,
      double timeElapsed, GameBean bean) {
  ...
  /**
   * Check if player flag hasWon is set to true Used in driver to check if game won/which splash
   * screen to display
   *
  ...
  /**
   * Check if player is not alive (alive status is false) Used in driver to check if game lost/which
  ...
  /**
   * Passes nodes of the (unmoving/unchanging) obstacles that should appear on the screen to the
   * Display
   *
  ...
  /**
   * Pass collection of MovableBounds that are all objects whose states can be changed from what was
   * initially loaded in. This is passed to display for initial loading in of values.
   *

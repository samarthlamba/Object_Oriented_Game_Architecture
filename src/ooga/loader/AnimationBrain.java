...
public class AnimationBrain {
  ...
  private static final List<AnimationState> ANIMATION_STATE_LIST = Arrays
      .asList(AnimationState.values());
  ...
  public AnimationBrain(String entityName) throws FactoryException {
    try {
      ...
      throw new FactoryException(
          String.format("Cannot build AnimationBrain for entity %s", entityName), e);
    }
  }
  ...
  /**
   * The white space constant in x is the number of pixels of white space between the end of one
   * sprite and the beginning of the next.
   *
  ...
  /**
   * Copies the position map, which specifies the x,y positions for the first sprite of each
   * AnimationState.
   *
   * @return a map whose keys are the 4 AnimationStates and whose values are their sprites first
   * positions
   */
  ...
  /**
   * Copies the length map, which specifies how many sprites comprise the animations for each
   * AnimationState.
   *
   * @return a map whose keys are the 4 AnimationStates and whose values are ints of the number of
   * sprites in those animations
   */

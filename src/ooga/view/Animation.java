...
/**
 * Performs the animation of given image based on information regarding the inputted image. Uses
 * viewport to determine what to show to the user
 */
public class Animation extends
    Transition { //fsm backend if separation    //look at unity // dont look at unity for the love of god
  ...
  public Animation(Image image, Double spriteWidth, Double spriteHeight, double xWhiteSpaceConstant,
      double yWhiteSpaceConstant, int lengthOfAnimation, int positionOfFirstAnimation,
      int framesPerRow, int actualSprintWidth, int actualSprintHeight, int duration) {
  ...
  protected void interpolate(double frac) {
    int index = Math.min((int) Math.floor(frac * length), length - 1) + positionOfFirstAnimation
        - 1; //the first part was a part of interpolate and the +positionOfFirstAnimation offsets things
    if (index != lastIndex) {
      double x = (index % framesPerRow) * actualWidth + xWhiteSpaceConstant * ((index
          % framesPerRow)); //current position in row * width of image + amount of white space to leave * the current position*2
      //System.out.println(x);
      final double y = (index / framesPerRow) * actualHeight + yWhiteSpaceConstant * (index
          / framesPerRow); //we expect this to be fine and this works as intended. It moves to the next row by this type of division
